...
function GetPendingReceive() {
	...
	objBO.CartId = '-';
	objBO.ItemId = '-';
	objBO.Logic = "LaundryPendingReceive";
	$.ajax({
		method: "POST",
		url: url,
		...
		success: function (data) {
			...
			$.each(data.ResultSet.Table, function (key, val) {
				...
				tbody += "<td style='width:10%'><input type='text' readonly='' style='height:20px;background:#cfffe4' class='form-control text-right' value='" + val.qty + "'/></td>";
...
function ReceiveLaundry() {
	if (confirm('Are you sure to Receive Selected Stocks?')) {
		...
		$('#tblPendingReceive tbody tr input:checkbox:checked').map(function () {
			objBO.push({
				...
				'frmcart': $(this).closest('tr').find('td:eq(2)').text(),
				'tocart': $(this).closest('tr').find('td:eq(2)').text(),
				'item_id': $(this).closest('tr').find('td:eq(3)').text(),
				'qty': $(this).closest('tr').find('td:eq(7)').find('input:text').val(),
				'Logic': 'LinenReceiving'
			});
		});
		$.ajax({
			method: "POST",
